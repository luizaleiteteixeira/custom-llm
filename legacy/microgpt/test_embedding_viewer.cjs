...
const checkpointBytes = fs.readFileSync(path.join(__dirname,'examples/llm_runs',bundle.run,'checkpoint.json'));
...
assert.equal(bundle.before.length,27);
assert.equal(bundle.after[0].length,16);
const probe=JSON.parse(fs.readFileSync(path.join(__dirname,'examples/llm_runs',bundle.run,'inspection.json')));
...
assert.equal(imported.before,null);
assert.equal(imported.tokens.at(-1),'BOS');
...
console.log(`Pooled PCA retains ${(fit.retained*100).toFixed(4)}% variance; token a neighbors after training:`,nearest(bundle.after,0).map(x=>[bundle.tokens[x.i],x.score]));
